...
- `/jobscout` -> guided flow: ask profession, then ask day range via inline keyboard, then dispatch GitHub
...
The guided `/jobscout` flow stores a short-lived command session in KV keyed by chat/user, so free-text profession input and the follow-up day-range callback can be stitched together safely.
