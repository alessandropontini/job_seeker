...
def fetch_remotive(since_days: int) -> list[JobPosting]:
...
def _fetch_remotive_payload() -> dict:
    with urllib.request.urlopen(REMOTIVE_API_URL, timeout=30) as response:
        return json.loads(response.read().decode("utf-8"))


def _parse_remotive_payload(payload: dict, since_days: int) -> list[JobPosting]:
...
def parse_remotive_payload(payload: dict, since_days: int) -> list[JobPosting]:
