...
- 2026-04-14: extended the interactive Telegram `/jobscout` flow with a runtime location-scope step (`Italia`, `Europa`, `USA`, `Mondo`) and added CLI/workflow support for `location_scope`, so manual searches can temporarily widen or narrow geography without editing the base config.
- 2026-04-14: expanded Telegram/CLI runtime profession focus from a single role to a comma-separated profession list with OR semantics, so manual searches can widen recall across adjacent roles such as `Data Architect, IT Solution Architect, Data Governance Manager` without weakening the static CV/domain gate.
- 2026-04-16: changed manual profession searches so the chosen profession list is the main gate and CV alignment becomes a visible ranking metric (`CV fit %`) instead of a hard blocker, allowing generic searches such as `Project Manager` or mixed-role searches to run without being suppressed by the static CV profile.
- 2026-04-16: hardened manual digests so the selected runtime area (`Italia`, `Europa`, `USA`, `Mondo`) is respected in the final Telegram/report selection, avoiding off-scope anti-zero fallback rows in the user-facing digest.
- 2026-04-16: expanded source coverage again with an Ashby public board connector plus curated default boards, activated conservative Lever defaults (`plaid`, `wealthfront`), and switched the Telegram/GitHub default source list to `remotive, wwr, arbeitnow, greenhouse, ashby` for higher recall on public hosted careers pages.
- 2026-04-16: hardened multi-source digest quality by adding light source/company diversity in Telegram selection and extra ranking bonuses when architecture, data-domain, and platform signals overlap in the same job posting.
